    ...
    try:
        ...
        while True:
            ...
            if user_input.lower() in ("quit", "exit"):
                print("\033[94m[Session ended. Goodbye!]\033[0m")
                break

            # Add user turn to history
            history.append({"role": "user", "parts": [{"text": user_input}]})

            # Keep only last 3 turns (6 entries: user + model alternating)
            if len(history) > 6:
                history = history[-6:]
            ...
            try:
                ...
                print(f"\n\033[91m[Error calling Gemini API: {e}]\033[0m")
        ...
        chat = model_inst.start_chat(history=[])
        turn_count = 0
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for entry in results:
        ...
        for model_key in ["gpt4o", "gpt4o_mini", "gemini_flash"]:
            ...
            cost = f"${model_result.get('cost', 0):.6f}"

            row = f"| {prompt} | {model} | {truncated} | {latency} | {tokens} | {cost} |"
            rows.append(row)
